...
class GT(nn.Module):
    def __init__(
        ...
        self.top_node_encoder = nn.Linear(hidden_dim * 2, hidden_dim)
    ...
    #     x_inter, x_intra = batched_data.x_inter, batched_data.x_intra
    #     x = torch.cat([x_intra, x_inter], dim=1)
    #     # graph_attn_bias
    ...
    #
    #     # graph_attn_bias
    #     # n_graph, n_node = x_intra.size()[:2]
    #     # graph_attn_bias = attn_bias.clone()
    #     node_intra_feature = self.node_encoder(x_intra)  # [n_graph, n_node, n_hidden]
    #     if perturb is not None:
    #         node_intra_feature += perturb
    #
    #     # global_node_feature = self.graph_token.weight.unsqueeze(0).repeat(n_graph, 1, 1)
    #     # node_intra_feature = torch.cat([node_intra_feature, global_node_feature], dim=1)
    #     #
    #     # graph_attn_bias = torch.cat(
    #     #     [graph_attn_bias, self.graph_token_virtual_distance.weight.unsqueeze(0).unsqueeze(2).
    #     #     repeat(n_graph, 1, n_node, 1)], dim=1)
    #     # graph_attn_bias = torch.cat(
    #     #     [graph_attn_bias, self.graph_token_virtual_distance.weight.unsqueeze(0).unsqueeze(0).
    #     #     repeat(n_graph, n_node + self.num_global_node, 1, 1)], dim=2)
    #
    #     # 第一步：同区内计算attention
    #     node_intra_feature = self.input_dropout(node_intra_feature)
    #
    #     for enc_layer in self.layers:
    #         # output = enc_layer(output, graph_attn_bias)
    #         node_intra_feature = enc_layer(node_intra_feature)
    #     node_intra_feature = self.final_ln(node_intra_feature)
    #
    #     # 第二步：不同区间计算attention
    #     node_inter_feature = self.node_encoder(x_inter)
    #     node_inter_feature[:, 0, :] = node_intra_feature[:, 0, :]
    #     if perturb is not None:
    #         node_inter_feature += perturb
    #
    #     output = self.input_dropout(node_inter_feature)
    #
    #     for enc_layer in self.layers:
    #         # output = enc_layer(output, graph_attn_bias)
    #         output = enc_layer(output)
    ...
    def forward(self, batched_data, perturb=None):
        ...
        node_inter_feature = self.input_dropout(node_inter_feature)

        for enc_layer in self.layers:
            # output = enc_layer(output, graph_attn_bias)
            node_inter_feature = enc_layer(node_inter_feature)

        # 拼接除了第一行的特征
        node_feature = torch.cat([node_intra_feature[:, :], node_inter_feature[:, 1:]], dim=1)

        # 处理第一行的特征（拼接后过Linear层降维）
        top_node_feature = self.top_node_encoder(
            torch.cat([node_intra_feature[:, :1], node_inter_feature[:, :1]], dim=2))
        node_feature[:, 0, :] = top_node_feature[:, 0, :]
        node_feature = self.final_ln(node_feature)

        # output part
        output = self.downstream_out_proj(node_feature[:, 0, :])
        return F.log_softmax(output, dim=1)
    ...
    #         # output = enc_layer(output, graph_attn_bias)
    #         node_intra_feature = enc_layer(node_intra_feature)
    #     node_feature = self.final_ln(node_intra_feature)
    #
    #     # output part
    #     output = self.downstream_out_proj(node_feature[:, 0, :])
    #     return F.log_softmax(output, dim=1)
